...
int main(int argc, char **argv) {
  ...
  { /* contiguous copy with differing base */
    Array<int, 1 Global> x(RD(1, 101));
    Array<int, 1 Global> y(RD(50, 151));
    upcxx_foreach (p, x.domain()) { x[p] = p[1]; };
    upcxx_foreach (p, y.domain()) { y[p] = p[1]+1000; };
    x.copy(y);
    upcxx_foreach (p, x.domain()) {
      int expected;
  ...
  { /* contiguous copy with non-trivial stride */
    ...
    Array<int, 1, strided Global> y =
      Array<int, 1>(RD(1, 11)).inject(POINTLIT(10));
    upcxx_foreach (p, x.domain()) { x[p] = p[1]; };
    upcxx_foreach (p, y.domain()) { y[p] = p[1]+1000; };
    x.copy(y);
    upcxx_foreach (p, x.domain()) {
  ...
  { /* tranpose from contiguous -> contiguous */
    Array<int, 2 Global> x(RD(PT(1, 11), PT(6, 21)));
    Array<int, 2, unstrided Global> y =
      Array<int, 2>(RD(PT(11, 1), PT(21, 6))).permute(POINTLIT(2,1));
    upcxx_foreach (p, x.domain()) { x[p] = p[1]*100+p[2]; };
    upcxx_foreach (p, y.domain()) { y[p] = p[1]*100+p[2]+1000; };
    x.copy(y);
    upcxx_foreach (p, x.domain()) {
  ...
  { /* equal sideFactor/stride ratio */
    Array<int, 2, strided Global> x(RD(PT(1, 11), PT(6, 21), PT(2, 1)));
    Array<int, 2 Global> y(RD(PT(1, 11), PT(6, 16)));
    upcxx_foreach (p, x.domain()) { x[p] = p[1]*100+p[2]; };
    upcxx_foreach (p, y.domain()) { y[p] = p[1]*100+p[2]+1000; };
    x.copy(y);
    upcxx_foreach (p, x.domain()) {
...

  {
    ...
    Array<int, 1 Global> x(RD(0, 7));
    upcxx_foreach (p, x.domain()) { x[p] = 100+p[1]; };
...

  {
    ...
    Array<int, 1 Global> x =
      Array<int, 1>(RD(0, 61)).constrict(RD(0, 61, 10));
    upcxx_foreach (p, x.domain()) { x[p] = 100+p[1]; };
