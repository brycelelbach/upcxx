...
LocalSparseMat::LocalSparseMat(ndarray<double, 1> la, ndarray<int, 1> lcidx,
                               ndarray<int, 1> lrow, int paramN,
  ...
  ndarray<int, 1> rowptr(RECTDOMAIN((rowStart), (rowEnd+2)));
  colidx = ndarray<int, 1>(RECTDOMAIN((0), (numNonZeros)));
  a = ndarray<double, 1>(colidx.domain());
  ...
  rowRectDomains = ndarray<RectDomain<1>, 1>(RectDomain<1>(rowptr.domain().min(),
                                                           rowptr.domain().max()));
...
void LocalSparseMat::multiply(ndarray<double, 1> output, ndarray<double, 1> input) {
  double sum;

  // make instance arrays into local arrays (BIG performance difference)
  ndarray<RectDomain<1>, 1> lrowRectDomains = rowRectDomains;
  ndarray<int, 1> lcolidx = colidx;
  ndarray<double, 1> la = a;
