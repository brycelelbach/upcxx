...
class LocalSparseMat {
 private:
  ...
  ndarray<RectDomain<1>, 1> rowRectDomains;  // generated from rowptr, indexed by row
  ndarray<int, 1> colidx;                    // column indices
  ndarray<double, 1> a;                      // sparse matrix values
 ...
 public:
  ...
  LocalSparseMat(ndarray<double, 1> la, ndarray<int, 1> lcidx,
  ...
  void multiply(ndarray<double, 1> output, ndarray<double, 1> input);
};
